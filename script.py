...
faces = faceCascade.detectMultiScale(grayImg, scaleFactor=1.05,
        minNeighbors=5)

for (x, y, width, height) in faces:
    img = cv2.rectangle(img, (x, y), (x + width, y + height), (0, 255,
                        0), 3)

resizedImg = cv2.resize(img, (int(img.shape[1] / 3), int(img.shape[0]
                        / 3)))
...
while True:
    ...
    thresholdFrame = cv2.threshold(deltaFrame, 30, 255,
                                   cv2.THRESH_BINARY)[1]
    ...
    for i in contours:
        ...
        cv2.rectangle(frame, (x, y), (x + width, y + height), (0, 255,
                      0), 3)
